...
class DataProcessor:
    ...
    def open_image():
        ...
        img_path = DataProcessor.ask_for_image() 
        ...
        cur.execute("SELECT EXISTS(SELECT 1 FROM images WHERE img_path = %s)", (img_path,))
        ...
        (polygons, connected_points, connected_inner_points, slider_values, matches, sacb), start_time, msg = DataProcessor.get_2d_data(img_path, xml_path, dbdata_exists, localdata_exists)
        VisualizationProcessor.setup_sliders(img_path, polygons, connected_points, connected_inner_points, slider_values, matches, sacb, start_time, msg)
    ...
    def ask_for_image():
        ...
        IMAGE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "orig_img"))
        image_path = filedialog.askopenfilename(
            initialdir=r"C:\Users\user\Downloads\2d_point_detection\orig_img",
            title="Select a File",
    ...
    def get_2d_data(img_path, xml_path, dbdata_exists, localdata_exists):
        if dbdata_exists and localdata_exists:
            cur.execute("SELECT uploaded_at FROM images WHERE img_path = %s", (img_path,))
            ...
            load_locally = DataProcessor.choose_load_location(db_timestamp, local_timestamp)
        else:
            load_locally = localdata_exists
        #save start time of load
        start_time = time.time()
        if load_locally:
            return DataProcessor.process_raw_points(img_path, xml_path), start_time, "Loaded Locally! Time Taken: "
        return DataProcessor.load_dbdata(img_path), start_time, "Loaded from Database! Time Taken: "
    ...
    def load_dbdata(img_path):
        ...
        #Loading points
        connected_points = DataProcessor.load_point_data(img_path, "connected_points")
        connected_inner_points = DataProcessor.load_point_data(img_path, "connected_inner_points")
    ...
    @staticmethod
    def load_point_data(img_path, table_name):
        cur.execute(f"""
    ...
    def save_dbdata(img_path, polygons, connected_points, connected_inner_points, slider_values, matches, sacb):
        ...
        DataProcessor.save_point_data("connected_points", connected_points, img_path)
        DataProcessor.save_point_data("connected_inner_points", connected_inner_points, img_path)
    ...
    def process_raw_points(image_path, xml_path):
        polygons, points, inner_points, debug_fits = DataProcessor.parse_xml(xml_path, image_path)
        width_2d = DataProcessor.get_2d_width(polygons)
        buffer_distance = 402
...
class StitchingProcessor:
    ...
    def k_nearest_neighbors(center_pt, candidates, k, polygons):
        ...
        """
        max_radius = DataProcessor.get_2d_width(polygons)/20
...
class VisualizationProcessor:
    ...
    def draw_visualization(vis_ax,
        ...
        # Stitching alignment
        stitching_alignment_to_check = StitchingProcessor.alignment_check(points, stitching_alignment_closest_boundary, stitching_alignment_candidates)
    ...
    def setup_sliders(img_path, polygons, connected_points, connected_inner_points, slider_values, matches, sacb, start_time, msg):
        mid_time = time.time()
        img = DataProcessor.load_image(img_path)
        width_2d = DataProcessor.get_2d_width(polygons)
        ...
        def update(val):
            ...
            s_comp.valtext.set_text(f"width_2d({int(width_2d)})/{int(s_comp.val)}")
        ...
        for slider in sliders:
            ...
            slider.set_val(list(slider_values.values())[sliders.index(slider)])
        ...
        def on_save_click(val):
            ...
            DataProcessor.save_dbdata(img_path, polygons, connected_points, connected_inner_points, updated_slider_values, matches, sacb)
